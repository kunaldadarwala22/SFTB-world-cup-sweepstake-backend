...
logger = logging.getLogger("sftb-sweepstake")
...
app = FastAPI(title="SFTB World Cup Sweepstake API")
...
async def compute_sweepstake():
    ...
    if final_match and final_match.get("status") == "FINISHED":
        ...
        if winner == "HOME_TEAM":
            ...
            pens = score.get("penalties", {})
            if pens.get("homeTeam") is not None and pens.get("awayTeay") is not None:
...
@app.get("/")
async def root():
    return {"message": "SFTB World Cup Sweepstake API. See /api/sweepstake"}
